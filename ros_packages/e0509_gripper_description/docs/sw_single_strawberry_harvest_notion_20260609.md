...
| MoveSplineJoint 실행 | `2건 모두 성공` | pre-approach, scan pose 복귀 |
| TOOL 기준 MoveLine 실행 | `3건 모두 성공` | final approach `20mm`, extra advance `65mm`, retreat `65mm` |
| gripper 접근 준비 상태 | position `600` | 노드 시작 및 pick 완료/실패 후 복귀 |
| BASE 기준 detach pull | `40mm`, 실행 성공 | 과실 분리를 위한 BASE `-Z` 직선 동작 |
...
>
> planning latency는 계획 결과가 나올 때까지 걸린 시간이다. **낮을수록 빠른
> 판단**이지만, 낮다고 성공 확률이 높거나 경로 품질이 좋다는 의미는 아니다.
...
- 상단: cuRobo가 검사한 후보별 계획 결과와 판단 시간
- 가운데: 최종 선택된 경로를 실제 로봇이 실행한 결과
- 하단: 수확 시작부터 파지, 분리, scan pose 복귀까지 `36.4초` 시간 흐름

그래프 용어:

| 용어 | 쉬운 설명 |
| --- | --- |
| spline-jump 거부 | 관절 각도가 `±360deg` 경계를 넘으며 갑자기 크게 회전할 위험 경로를 실행 전에 차단한 것. 수확 성공은 아니지만 안전장치가 정상 작동한 결과 |
| MoveSplineJoint `2/2 성공` | cuRobo가 선택한 관절 경로로 pre-approach 이동과 scan pose 복귀를 각각 정상 실행 |
| TOOL MoveLine `3/3 성공` | 그리퍼가 바라보는 방향 기준으로 최종 진입, 추가 진입, 후퇴를 정상 실행 |
