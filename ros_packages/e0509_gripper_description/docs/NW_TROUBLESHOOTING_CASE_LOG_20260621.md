...
| 9 | 같은 J2 한도 초과가 **2단계 분리 이후에도** 재발(다른 타겟) | 원인 재진단 → 후진 leg가 raw Cartesian이라 IK 브랜치가 불안정 | ⚠️ 수정함, 실기 미검증 |
...
**결과**: ⚠️ 코드 변경·빌드·import 체크까지 통과했지만 **실기 검증 아직 안 함**. 다음 실기에서
같은 타겟 재현해 `RETREAT_BASE`가 cuRobo plan으로 정상 완료되는지, 또는 plan 자체가 실패해서
안전하게 hold하는지 확인 필요.
...
- 항목 9(cuRobo 기반 retreat) 실기 검증 — 같은 타겟 재현.
