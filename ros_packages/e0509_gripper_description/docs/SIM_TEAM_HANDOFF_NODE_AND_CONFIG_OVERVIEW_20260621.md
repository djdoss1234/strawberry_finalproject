...
## 0. ROS2 기본 개념 — 완전 처음 보는 사람용 (2026-06-21 추가)

ROS도 rqt_graph도 처음이면 이 절만 읽고 3절(노드 표)/5절(토픽 표)로 가면 된다.

### 0-1. 노드(Node) = "일하는 사람 한 명"

**노드는 그냥 켜져서 돌고 있는 프로그램 하나**다. 우리 프로젝트에는 이런 "일하는
사람"이 세 명 있다:

- **`strawberry_fusion_node`** = "카메라 보는 사람" — 카메라로 딸기를 찾고 "여기 딸기
  있어요!"라고 알려줌
- **`scan_executor_node`** = "안내하는 사람" — 로봇팔을 이 칸 저 칸으로 움직이면서
  "지금 여기 보고 있어요", "다음은 여기로 가요"를 정함
- **`curobo_planner_node`** = "팔 움직이는 사람" — "여기 딸기 있다"는 말을 듣고
  실제로 팔을 뻗어서 잡음

이 셋 말고도 그림 그려주는 사람(`rviz2`, `workspace_marker_node`), 로봇 팔 자체를
움직여주는 사람(두산 회사가 만든 `dsr_controller2`), 그리퍼(손가락)를 움직여주는
사람(`gripper_service_node`) 같은 "보조 인력"도 같이 돌아간다. **이번에 한 리팩토링은
이 사람들 숫자를 하나도 안 늘리고 줄이지도 않았다** — 그냥 "팔 움직이는 사람" 머릿속
업무 노트(코드 파일)를 정리정돈한 것뿐이다.

### 0-2. 토픽(Topic) = "교실 게시판에 붙이는 쪽지"

누가 게시판에 쪽지를 붙이면(`publish`, "발행"), 그 게시판을 보고 있는 사람은 전부
다 그 내용을 본다(`subscribe`, "구독"). **답장은 안 한다.** 그냥 계속 붙이기만 함.

- 예: `strawberry_fusion_node`가 `/strawberry/detection/pick_pose`라는 게시판에
  "딸기가 (x,y,z)에 있어요"라고 계속 붙임 → `curobo_planner_node`가 이 게시판을
  보고 있다가 새 쪽지가 오면 잡으러 감.
- 한 게시판을 여러 명이 동시에 봐도 된다 — `/dsr01/joint_states`(지금 로봇팔이 몇
  도 꺾여있는지)는 세 핵심 노드가 전부 같이 본다.

### 0-3. 서비스(Service) = "짧은 질문하고 바로 답 받기"

전화로 "지금 몇 시예요?" 물어보고 바로 "3시요" 듣는 것. **물어본 사람은 답이 올
때까지 기다린다**(다른 일을 못 함). 빨리 끝나는 일에 씀.

- 예: `curobo_planner_node`가 `/gripper_service/set_position`으로 "그리퍼를 600
  위치로 옮겨줘"라고 묻고, 그리퍼 쪽에서 "옮겼어요"라고 답할 때까지 기다림.

### 0-4. 액션(Action) = "택배 주문하고 배송 추적하기"

서비스랑 비슷하게 "부탁하고 결과 받기"이긴 한데, **오래 걸리는 일이라 중간에
"지금 어디까지 했어요"를 계속 알려준다.** 그리고 "주문 취소"도 할 수 있다.

- 예: `/gripper_service/safe_grasp`(안전하게 잡기) — 그리퍼를 천천히 닫으면서
  "지금 위치 650", "660", "딸기 잡힌 것 같음" 같은 중간 소식을 계속 주고, 다 닫히면
  마지막으로 "성공/빈손" 최종 결과를 줌. 한 번 묻고 끝나는 서비스로는 이런 중간
  진행 상황을 못 줌.

### 0-5. rqt_graph 보는 법 — 도형/화살표

rqt_graph는 "누가 어느 게시판에 글을 쓰고 누가 그걸 보는지"를 그림으로 그려주는
도구다(서비스/액션은 기본 화면엔 잘 안 나옴 — 토픽 위주).

- **타원(동그란 모양)** = **노드** = 일하는 사람 한 명
- **사각형** = **토픽** = 게시판
- **화살표** = 누가 누구한테 정보가 흘러가는지. 화살표가 **노드 → 토픽**이면
  "이 사람이 이 게시판에 글을 씀"(발행), **토픽 → 노드**면 "이 사람이 이 게시판을
  보고 있음"(구독).
- 예를 들어 `/strawberry/detection/pick_pose`라는 사각형으로 `strawberry_fusion_node`
  타원에서 화살표가 들어가고, 그 사각형에서 다시 `curobo_planner_node`로 화살표가
  나가면 → "fusion이 써서, curobo가 읽는다"는 뜻.
- 우리 프로젝트 rqt_graph를 직접 캡처해서 본 결과(2026-06-21), 실제로 5절 표와
  토픽 이름·방향이 전부 일치하는 것을 확인했다 — 즉 이 문서 내용이 실제 그래프와
  맞다.

이 정도만 알면 5절 표("Publisher/Client" 칸 = 게시판에 글 쓰는 사람,
"Subscriber/Server" 칸 = 그 게시판을 보는 사람)를 바로 읽을 수 있다.
