...
MAX_MOVEJ_DELTA_DEG = [180.0, 120.0, 140.0, 120.0, 140.0, 180.0]
...
class JointJogControl(Node):
    def __init__(self):
        ...
        self.cli_gripper_set_position = (
            self.create_client(SetPosition, "/gripper_service/set_position")
            if SetPosition is not None else None
        )
    ...
    def send_movej(self, joints_deg, sync_type=0, wait=True):
        if not self.cli_movej.wait_for_service(timeout_sec=0.5):
            ...
            return False
        joints_deg = self.safe_equivalent_joints(joints_deg)
    ...
    def safe_equivalent_joints(self, target_deg):
        cur = self.current_joints_deg()
        adjusted = [float(v) for v in target_deg]
        if cur is None:
            return adjusted
        for idx in WRAP_EQUIVALENT_JOINT_IDX:
            lo, hi = JOINT_LIMITS_DEG[idx]
            base = adjusted[idx]
            candidates = [base + 360.0 * k for k in range(-2, 3)]
            candidates = [c for c in candidates if lo <= c <= hi]
            if candidates:
                adjusted[idx] = min(candidates, key=lambda c: abs(c - cur[idx]))
        deltas = [abs(a - c) for a, c in zip(adjusted, cur)]
        bad = [
            f"J{i + 1} {d:.1f}>{MAX_MOVEJ_DELTA_DEG[i]:.1f}"
            for i, d in enumerate(deltas)
            if d > MAX_MOVEJ_DELTA_DEG[i]
        ]
        if bad:
            print(
                "MoveJoint blocked: excessive joint swing "
                + ", ".join(bad)
                + f" | current={[round(v, 1) for v in cur]} "
                + f"target={[round(v, 1) for v in adjusted]}"
            )
            return None
        return adjusted
    ...
    def set_gripper_position(self, pos, label=None):
        if self.cli_gripper_set_position is None:
            print("gripper SetPosition interface not available. Build/source dsr_gripper_tcp_interfaces.")
            return
        if not self.cli_gripper_set_position.wait_for_service(timeout_sec=1.0):
            print("/gripper_service/set_position not available")
            return
        ...
        if res and res.success:
            name = label or f"position {pos}"
            print(f"gripper {name} OK: {getattr(res, 'message', '')}")
        else:
            msg = res.message if res else "timeout"
            name = label or f"position {pos}"
            print(f"gripper {name} FAIL: {msg}")
    ...
    def handle_key(self, key, raw_term):
        if key in ["1", "2", "3", "4", "5", "6"]:
            ...
            self.set_gripper_position(600, "open(600)")
        elif key == "c":
            self.set_gripper_position(700, "close(700)")
        elif key == "f":
            with CookedInput(raw_term):
                val_str = input("\nGripper position (0~700): ").strip()
            try:
                val = int(val_str)
                if 0 <= val <= 700:
