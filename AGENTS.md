...
2. `docs/CURRENT_STATUS.md`에서 마지막 완료 상태와 다음 사용자 입력/작업을 확인합니다.
3. 이 파일에서 설계 원칙과 작업 우선순위를 확인합니다.
4. `docs/project_scope.md`에서 interface 및 평가 항목을 확인합니다.
5. `docs/development_roadmap.md`에서 전체 단계와 당장 진행할 작업을 확인합니다.
6. `docs/project_recording_system.md`와 `docs/development_workflow.md`를 확인합니다.
7. `docs/visual_asset_guide.md`에서 아직 확보하지 않은 시각자료를 확인합니다.
8. 최근 `docs/worklogs/`, `docs/runs/`, `docs/issues/`와
   `docs/portfolio_evidence.md`를 확인합니다.
9. `git status`, `git log --oneline --decorate -10`, 최근 commit diff를 확인합니다.
10. 코드가 추가된 이후에는 관련 launch/config/module과 최근 실험 문서를 확인합니다.
11. 이전 구현의 동작 방식이 필요할 때만 `strawberry_miniproject`를 참조합니다.
