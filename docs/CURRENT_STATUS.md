...
최종 갱신일: 2026-06-01 (스캔→픽 통합, 손상 최소화 구현 완료)
...
| Blocker | 상태 | 대기 중인 것 |
|---------|------|------------|
| YOLO .pt 학습 파일 | 재학습 진행 중 | 딸기 bbox + 줄기 접근방향 출력 형태 |
| 줄기 파지 파라미터 | 미티칭 | .pt 완성 후 DART로 실제 줄기 접근 각도 측정 |
| 스캔 포즈 그리퍼/카메라 각도 이슈 | 관찰 중 | 셀 중심 기준을 카메라→그리퍼 방향으로 수정 예정 |
...
### 즉시 (YOLO .pt 완성되면)
1. **YOLO 출력 포맷 확인**: bbox만인지 + 줄기 접근방향 벡터인지
   - 2D angle인지 3D vector인지 → TF 변환 필요 여부 결정
2. **줄기 접근방향 → 그리퍼 quaternion 변환** 구현
   - 현재 `WALL_QUAT_WXYZ` 고정값 → 줄기 방향별 동적 계산으로 교체
3. **YOLO 노드 연결**: `/strawberry/detection/pick_pose` + `/strawberry/detection/scene_positions` 발행
4. **줄기 파지 DART 티칭**: `GRASP_Z_BIAS`, `GRASP_OFFSET` 재조정
5. **첫 실제 수확 시도** + KPI 기록 시작

### 이후
6. **pickability 필터** 구현 (depth std, 이웃 거리, 가장자리 여백)
7. **그리퍼 피드백** (FlangeSerialRead reg 281 → goal vs actual)
8. **스캔 포즈 각도 수정** (카메라→그리퍼 방향 정렬, VLA 협의 후)
9. **MoveIt trajectory 검증** (cuRobo 궤적 replay)
10. Place slot 전체 티칭 (현재 slot0만)
...
| 파일 | 역할 |
|------|------|
| `config/scan_pose_candidates_refit_candidate.yaml` | v11 스캔 포즈 |
