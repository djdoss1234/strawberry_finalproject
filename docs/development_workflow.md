...
`realsense_alignment_viewer`는 camera 표시 전용입니다. 이전에 추가한
Cartesian step control은 joint-limit safety guard 없이 실제 motion을
호출해 사고가 발생했으므로 철회했습니다. 앞으로 visualization utility가
